...
const defaultAuthor = {
  ...
  bio: 'Frontend engineer yang fokus pada performa web, UI yang rapi, dan pengalaman pengguna yang mudah dipahami.',
}
...
function enhanceHeadings(contentHtml) {
  const tableOfContents = []

  const updatedHtml = contentHtml.replace(/<h([2-3])>(.*?)<\/h\1>/g, (_, level, headingHtml) => {
...
export async function getPostData(id) {
  ...
  const enhancedContent = enhanceHeadings(contentHtml)
  const fallbackMeta = postMetaMap[id] ?? {
    category: 'Umum',
    tags: ['artikel'],
  }
  ...
  return {
    ...
    category: matterResult.data.category ?? fallbackMeta.category,
    tags: matterResult.data.tags ?? fallbackMeta.tags,
    contentHtml: enhancedContent.contentHtml,
    tableOfContents: enhancedContent.tableOfContents,
    author: matterResult.data.author ?? defaultAuthor,
    ...matterResult.data,
  }
}
